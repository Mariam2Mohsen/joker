...
const MyServicesPage = () => {
  ...
  const fetchServices = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:5000/api/provider/services/my-services", {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const res = await axios.patch(`http://localhost:5000/api/provider/services/${id}/toggle`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
                    ...
                    <div className="flex justify-between items-start mb-10 relative z-10">
                       <div className="relative group/avatar">
                          <div className="w-20 h-20 rounded-[2rem] bg-[#102C57] text-[#FEFAF6] flex items-center justify-center font-black text-xl shadow-2xl transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-700 relative overflow-hidden border-4 border-white">
                             {svc.image ? (
                               <img src={`http://localhost:5000/images/${svc.image}`} alt="" className="w-full h-full object-cover" />
                             ) : svc.service_name[0]}
