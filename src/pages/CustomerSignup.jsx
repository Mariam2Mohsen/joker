  ...
  const handleSignUp = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/api/users/user_signup",
        {
        ...
        <div className="w-full bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(16,44,87,0.06)] overflow-hidden border border-[#EADBC8]/50">
          ...
          <form onSubmit={handleSignUp} className="p-8 md:p-12 space-y-8">
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Input label="Password" type="password" required placeholder="Password" value={form.password || ''} error={errors.password} onChange={e => { setForm({ ...form, password: e.target.value }); if (errors.password) setErrors({ ...errors, password: null }) }} />
                <Input label="Confirm Password" type="password" required placeholder="Confirm Password" value={form.confirmPassword || ''} error={errors.confirmPassword} onChange={e => { setForm({ ...form, confirmPassword: e.target.value }); if (errors.confirmPassword) setErrors({ ...errors, confirmPassword: null }) }} />
              </div>
