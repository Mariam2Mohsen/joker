...
const ServiceList = () => {
  ...
  const fetchServices = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://joker-hm0k.onrender.com/api/admin/services', {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      ...
      await axios.patch(`https://joker-hm0k.onrender.com/api/admin/services/${id}/status`,
        { status: newStatus },
        { headers: getAuthHeader() }
      );
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`https://joker-hm0k.onrender.com/api/admin/services/${deleteModal.serviceId}`, {
        headers: getAuthHeader()
      });
                    ...
                    <td className="py-5 px-4">
                      <div className="flex justify-center">
                        <button
                          ...
                          className={`w-10 h-5 flex items-center rounded-full p-1 transition-all duration-300 ${service.status === 'Active' ? 'bg-green-500 shadow-sm' : 'bg-[#EADBC8]'
                            }`}
                        >
                          <div className={`w-3 h-3 bg-white rounded-full transition-all shadow-sm ${service.status === 'Active' ? 'translate-x-5' : 'translate-x-0'
                            }`}></div>
                        </button>
                      </div>
                    </td>
                    <td className="py-5 px-4 text-center">
                      <div className="relative flex justify-center">
                        ...
                        {activeActionId === service.service_id && (
                          <div className="absolute right-full mr-2 top-0 -translate-y-1/2 bg-white rounded-xl shadow-2xl border border-[#EADBC8] overflow-hidden z-[100] min-w-[140px]">
