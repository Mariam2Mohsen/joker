...
import { Edit3, Trash2, Plus, Search, ChevronLeft, ChevronRight, Settings, Users, ShieldAlert } from 'lucide-react';
...
const UsersPage = () => {
  ...
  if (loading) return <div className="p-10 text-center font-bold text-[#102C57] uppercase tracking-widest text-[10px]">Loading Personnel...</div>;
    ...
    <div className="p-4 md:p-8 bg-[#FEFAF6] min-h-screen">
        ...
        <div className="p-6 flex flex-wrap justify-between items-center gap-4 bg-white border-b border-[#FEFAF6]">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="border border-[#DAC0A3] rounded-lg px-4 py-2 text-[10px] font-black outline-none text-[#102C57] bg-[#FEFAF6] uppercase tracking-widest"
            >
            ...
            <tbody className="divide-y divide-[#FEFAF6]">
              {currentUsers.length > 0 ? currentUsers.map((user) => (
                <tr key={user.Users_id} className="hover:bg-[#FEFAF6]/50 transition-colors group">
                  <td className="py-5 px-8 text-[#102C57] text-xs font-black">{user.Full_Name}</td>
                    ...
                    <div className="flex justify-center">
                      <button 
                        ...
                        className={`w-11 h-5 flex items-center rounded-full p-1 transition-all duration-300 ${
                          user.account_status?.toLowerCase() === 'approved' ? 'bg-green-500 shadow-sm shadow-green-100' : 'bg-[#EADBC8]'
                        }`}
                      >
                        <div className={`w-3.5 h-3.5 bg-white rounded-full transition-all shadow-sm ${
                          user.account_status?.toLowerCase() === 'approved' ? 'translate-x-5' : 'translate-x-0'
                        }`}></div>
                      </button>
                    </div>
                  </td>
                  <td className="py-5 px-8 text-center">
                    <div className="relative flex justify-center">
                      ...
                      >
                        <Settings size={14} className={activeActionId === user.Users_id ? 'animate-spin-slow' : ''} />
                      </button>
        ...
        <div className="p-6 flex justify-between items-center bg-white border-t border-[#F2EBE4]">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="text-slate-300 hover:text-[#102C57] disabled:opacity-20"
            >
              <ChevronLeft size={16} />
            </button>
            ...
            {[...Array(totalPages)].map((_, i) => (
              <button
                ...
                className={`w-7 h-7 rounded-md text-[11px] font-black transition-all ${
                  currentPage === i + 1 ? 'bg-[#102C57] text-white shadow-md shadow-[#102C57]/20 scale-110' : 'text-slate-400 hover:bg-[#FDFCFB]'
                }`}
              >
                {i + 1}
              </button>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 bg-[#102C57]/40 backdrop-blur-sm z-[250] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-10 max-w-sm w-full shadow-2xl border border-[#EADBC8] text-center animate-scaleIn">
            <div className="bg-red-50 w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-8 text-red-500 shadow-inner">
              <ShieldAlert size={32} />
            </div>
