...
const AddUser = () => {
          ...
          if (response.data.success) {
            ...
            if (user) {
              setFormData({
                ...
                userType: user.Role || (user.role_id === 1 ? 'Admin' : 'Admin_user'),
                email: user.Email || '',
                password: '',
                confirmPassword: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        Role: formData.userType === 'Admin' ? 'Admin' : 'Admin_user',
  ...
  const getFieldClass = (fieldName) => {
    return `${inputBaseClass} ${errors[fieldName] ? 'border-[#E72929] bg-[#FFF5F5]' : 'border-[#DAC0A3]'}`;
  };
