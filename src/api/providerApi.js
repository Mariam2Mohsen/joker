...
export const fetchAllProviders = async () => {
    try {
        ...
        return activeProviders.map(provider => {
            const avatar = provider.profile_image 
                ? (provider.profile_image.startsWith('http') ? provider.profile_image : `https://joker-hm0k.onrender.com/images/${provider.profile_image}`)
